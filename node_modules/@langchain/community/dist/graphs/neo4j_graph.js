...
var Neo4jGraph = class Neo4jGraph {
	...
	async addGraphDocuments(graphDocuments, config = {}) {
		...
		if (baseEntityLabel) {
			if (!(this.structuredSchema?.metadata?.constraint?.some((el) => JSON.stringify(el.labelsOrTypes) === JSON.stringify([BASE_ENTITY_LABEL]) && JSON.stringify(el.properties) === JSON.stringify(["id"])) ?? false)) {
				await this.query(`
